...
- [x] Modelo `Session` no Prisma (ver abaixo)
- [x] Migration
- [x] `POST /api/sessions` — criar sessão
- [x] `GET /api/sessions` — listar do usuário (filtros: patientId, status, area, data)
- [x] `GET /api/sessions/:id` — buscar sessão com SOAP completo
- [x] `PUT /api/sessions/:id` — atualizar (editar SOAP ou status)
- [x] `DELETE /api/sessions/:id` — soft delete (`isActive = false`)
- [x] Módulo `server/modules/sessions/` completo (application, domain, http, infra)
- [x] Validação Zod em todos os endpoints

### Frontend

- [x] Página `/atendimentos` — lista cronológica de sessões (design: lista com badges de status)
- [x] Página `/agenda` — próximos agendamentos (design: agenda-item cards com hora + duração)
- [x] Página `/pacientes/:id/sessoes/nova` — formulário de registro SOAP
- [x] Componente `SessionCard` (agenda-item do design system)
- [x] Componente `StatusBadge` (Agendado | Realizado | Cancelado)
- [x] Badge "Domiciliar" em sessões `HOME_CARE`
- [x] Link "Registrar atendimento" na ficha do paciente (Phase 3)
...
- [x] `createSession`: dados válidos, `patientId` de outro usuário (deve lançar erro)
- [x] `updateSession`: campos SOAP parciais salvos corretamente
- [x] `listSessions`: isolamento multi-tenant verificado

## Observações

- O código da phase 4 está implementado localmente.
- Para validar no browser local, é necessário reiniciar o `npm run dev` após o `prisma generate`, senão o processo aberto continua sem o modelo `Session` no client do Prisma.
